...
uint8_t PcdRequestB(uint8_t req_code, uint8_t AFI, uint8_t N, uint8_t *ATQB)
{
		...
		status = PcdComMF522(PCD_TRANSCEIVE,ucComMF522Buf,3,ucComMF522Buf,&unLen,5);
...
uint8_t GetIdcardNum(uint8_t *pid)
{
    ...
    ucComMF522Buf[2] =0x00; //((req_code<<3)&0x08) | (N&0x07);  // PARAM
		...
		status = PcdComMF522(PCD_TRANSCEIVE,ucComMF522Buf,5,ucComMF522Buf,&unLen,5);
    if (status == MI_OK) 
    {	
...
uint8_t ReadSr176(uint8_t addr, uint8_t *readdata)
{
    ...
    status = PcdComMF522(PCD_TRANSCEIVE,ucComMF522Buf,2,ucComMF522Buf,&unLen,20);
...
uint8_t ProtectSr176(uint8_t lockreg)
{
    ...
    ucComMF522Buf[3] = lockreg;
		...
		status = PcdComMF522(PCD_TRANSCEIVE,ucComMF522Buf,4,ucComMF522Buf,&unLen,5);
