...
{
    sFLASH_LowLevel_Init();//IO初始化

    /*!< Deselect the FLASH: Chip Select high */
    sFLASH_CS_HIGH();//CS拉高

    /*!< SPI configuration */
    SPI_InitStructure.DataDirection = SPI_DIR_DOUBLELINE_FULLDUPLEX; //4线全双工
    SPI_InitStructure.SpiMode       = SPI_MODE_MASTER;//主模式
    SPI_InitStructure.CLKPOL        = SPI_CLKPOL_HIGH;//时钟极性
    SPI_InitStructure.CLKPHA        = SPI_CLKPHA_SECOND_EDGE;//时钟相位
    SPI_InitStructure.NSS           = SPI_NSS_SOFT;//NSS由硬件还是软件控制

    SPI_InitStructure.BaudRatePres = SPI_BR_PRESCALER_4;//SPI时钟分频因子
...
void sFLASH_EraseSector(uint32_t SectorAddr)
{
    ...
    sFLASH_CS_LOW();
    /*!< sFLASH_CMD_SE Send Sector Erase instruction sFLASH_CMD_CE*/
  
 // sFLASH_SendByte(sFLASH_CMD_CE); /本指令是整片擦除，请谨慎开启
    sFLASH_SendByte(sFLASH_CMD_SE);//扇区
    //  sFLASH_SendByte(sFLASH_CMD_BE);//Block
...
/**************************   flash批量写入， 需提前整片擦除一次 *************/
void FLASH_WriteBuffer_batch(uint8_t *pBuffer, uint32_t WriteAddr, uint32_t NumByteToWrite)
{
    uint16_t NumOfPage = 0, NumOfSingle = 0, Addr = 0, count = 0, temp = 0, NumSector = 0, NumBlock = 0;

    Addr        = WriteAddr % sFLASH_SPI_PAGESIZE;
    count       = sFLASH_SPI_PAGESIZE - Addr;
    NumOfPage   = NumByteToWrite / sFLASH_SPI_PAGESIZE;//页数
    NumOfSingle = NumByteToWrite % sFLASH_SPI_PAGESIZE;//单页数量
    NumSector   = NumOfPage / 16; //扇区
    NumBlock    = NumSector / 16; //块
...
////////////////////////////////////////////////////////////
        if (NumOfPage == 0) //  小于1页
        {
            sFLASH_WritePage(pBuffer, WriteAddr, NumByteToWrite);//页写入
        }
        else //跨页
        {
            while (NumOfPage--)//跨页写
...
void SPI_PageWrite(uint32_t WriteAddr)
{
    /* Enable the write access to the FLASH */
    sFLASH_WriteEnable();
    /* Select the FLASH: Chip Select low */
...
void SPI_DMA_BufferRead(uint32_t ReadAddr)
{
    ...
    /*!< Disable the sFLASH_SPI  */
    SPI_Enable(sFLASH_SPI, DISABLE);//禁用spi

    /*!< Set the sFLASH_SPI Doubleline FullDuplex*///双工
    SPI_InitStructure.DataDirection = SPI_DIR_DOUBLELINE_FULLDUPLEX;
    SPI_Init(sFLASH_SPI, &SPI_InitStructure);
    /*!< Enable the sFLASH_SPI  */
    SPI_Enable(sFLASH_SPI, ENABLE);
...
//NumByteToRead:要读取的字节数(最大65535)1024 2048 4096 8192 512 256 128
