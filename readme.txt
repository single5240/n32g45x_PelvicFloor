...
治疗脉冲保留向下计数 PWM 方案。TIM1/TIM8 仅在对应治疗通道强度非零时启动计数器及 CC3 比较中断，不开启 UPDATE 和 CC4 中断。TIM1 CC3=8000，在 PWM1 有效脉冲开始前 200 us 的无效窗口内预选下一桥臂，避免在有效脉冲期间切换极性；TIM8 CC3=3200，保持在下一次重装前 200 us 预选下一互补桥臂。每相周期为 625 us，目标有效脉宽为 300 us，其余时间两桥臂无有效输出。CH2 因使用 TIM8_CH1N/CH2N，继续按互补输出采用等效 PWM 模式。通道归零、模式切换、关机或故障关断时立即关闭对应定时器中断并清除外设和 NVIC 挂起标志。
...
- 桥臂换向仅使用 TIM1/TIM8 的 CC3 比较中断：TIM1 在脉冲开始前 200 us 预选，TIM8 在重装前 200 us 预选，UPDATE 只作为硬件计数重装事件，不产生 ISR。
- 每个通道的两侧桥臂必须互斥，禁止同时导通。

### 压力与电池

- ADC1 用于电池电压及 PA6 外部参考采样；ADC2 用于 PA2 压力传感器采样。
- 电池电压先经 3:1 一阶低通；蓝牙百分比以 LCD 格数阈值为锚点，在 0%/33%/66%/100% 四个节点间线性插值。百分比采用 2% 死区和连续 3 次确认；未充电只允许下降，充电只允许上升。
- 未接充电器时，滤波电压严格低于 3.50 V 连续满 10 s（治疗模式）或 60 s（压力模式）后，系统先统一关闭治疗、气泵、阀门和蓝牙，再整屏闪烁 3 次、蜂鸣 3 次并进入关机低功耗。关机状态长按电源键会先检查电量；低于 3.50 V 时仅显示空电池图标闪烁 3 次，不进入工作状态。充电器接入会取消该低电流程。
- 压力流程为“空闲→预充气→正式测试→结果保持→放气复位”：首次有效采样达到 5 mmHg 后开始固定 10 s 测试，测试期间气泵以 500 Hz PWM 继续充气；电池电压 4.20 V 至 3.50 V 时占空比由 25% 线性补偿至 27%，压力值闪烁并显示实时值。
...
- 治疗和压力业务统一使用 `MODE_CONTROL` 绝对设定；治疗 `START_THERAPY(action=0x07)` 仅供蓝牙使用，会启动当前选中通道的 PWM 并强制 DAC 为 0，后续设置非零强度时 PWM 不间断。CH1/CH2 的远程预启动与强度调整互不影响；`POWER_OFF(0x09)` 为独立关机命令，`UI_ACTION(0x10)` 已停止支持。危险动作仍要求模式匹配、未处于充电互锁且 PB7 物理连接有效；断链会走统一停止路径。该 PWM/DAC=0 路径须完成台架验证后方可作为量产行为。
- 充电接入是本地与蓝牙共用的关机互锁：无论当前处于何种业务状态，均立即关闭两路治疗、DAC、气泵、电磁阀和蓝牙，并切换到仅显示充电动画或满电图标的关机充电状态。充电期间所有按键均不触发开机或业务操作；拔出充电器后关闭 LCD 与背光并直接进入关机低功耗流程。
...
以下宏位于 `inc/main.h`，气泵 PWM 相关宏位于 `inc/init.h`；修改前必须完成相应的示波器或台架验证。
...
| `MOTOR_PWM_FREQUENCY_HZ` | `500` | 气泵 TIM4_CH4 PWM 频率；128 MHz 定时器时钟、周期计数 12800 对应 20 分频。 |
| `MOTOR_PWM_DUTY_HIGH_VOLTAGE_PERCENT` / `MOTOR_PWM_DUTY_LOW_VOLTAGE_PERCENT` | `25` / `27` | 气泵 TIM4_CH4 在高/低电压端的 PWM 占空比，随电池电压线性补偿。 |
...
| `APP_LOW_BATTERY_SHUTDOWN_MV` | `3500` | 自动关机和开机禁止使用的电压阈值（mV）；严格低于该值时生效。 |
| `APP_LOW_BATTERY_THERAPY_CONFIRM_MS` | `10000` | 治疗模式下滤波电压连续低于阈值的确认时间。 |
| `APP_LOW_BATTERY_PRESSURE_CONFIRM_MS` | `60000` | 压力模式下滤波电压连续低于阈值的确认时间。 |
| `APP_LOW_BATTERY_FLASH_HALF_PERIOD_MS` / `APP_LOW_BATTERY_FLASH_COUNT` | `500` / `3` | 低电提示闪烁半周期和闪烁次数。 |
| `APP_BATTERY_FIRST_SAMPLE_SETTLE_MS` | `500` | BATEN 拉高后首次电池 ADC 采样的最小建立时间（ms）。 |
| `APP_BATTERY_BOOT_VALID_SAMPLE_COUNT` | `3` | 开机电量检查所需的有效采样次数；采样结果经过现有低通滤波后再判定。 |
| `APP_BATTERY_BOOT_CHECK_TIMEOUT_MS` | `3000` | 开机电量检查超时；超时按低电处理，禁止进入工作状态。 |
| `APP_BATTERY_PERIODIC_LOG_ENABLE` | `0` | 关闭每秒电池 ADC、参考 ADC、换算电压和电量日志；联调时可置 `1`。 |
